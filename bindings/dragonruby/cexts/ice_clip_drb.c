...
static void *drb_ffi__ZTSPv_FromRuby(mrb_state *state, mrb_value self) {
    ...
    drb_typecheck_aggregate_f(state, self, klass, &ForeignObjectType_ZTSPv);
...
static mrb_value drb_ffi_ice_clip_use_native_activity_Binding(mrb_state *state, mrb_value value) {
    ...
    if (argc != 1)
        mrb_raisef_f(state, drb_getargument_error_f(state), "'ice_clip_use_native_activity': wrong number of arguments (%d for 1)", argc);
...
static mrb_value drb_ffi_ice_clip_use_window_Binding(mrb_state *state, mrb_value value) {
    ...
    mrb_get_args_f(state, "*", &args, &argc);
    if (argc != 1)
        mrb_raisef_f(state, drb_getargument_error_f(state), "'ice_clip_use_window': wrong number of arguments (%d for 1)", argc);
...
static mrb_value drb_ffi_ice_clip_get_Binding(mrb_state *state, mrb_value value) {
    ...
    mrb_get_args_f(state, "*", &args, &argc);
    if (argc != 0)
        mrb_raisef_f(state, drb_getargument_error_f(state), "'ice_clip_get': wrong number of arguments (%d for 0)", argc);
...
static mrb_value drb_ffi_ice_clip_set_Binding(mrb_state *state, mrb_value value) {
    ...
    mrb_get_args_f(state, "*", &args, &argc);
    if (argc != 1)
        mrb_raisef_f(state, drb_getargument_error_f(state), "'ice_clip_set': wrong number of arguments (%d for 1)", argc);
...
static mrb_value drb_ffi_ice_clip_clear_Binding(mrb_state *state, mrb_value value) {
    ...
    mrb_get_args_f(state, "*", &args, &argc);
    if (argc != 0)
        mrb_raisef_f(state, drb_getargument_error_f(state), "'ice_clip_clear': wrong number of arguments (%d for 0)", argc);
...
#if defined(ICE_CLIP_ANDROID)
    mrb_define_module_function_f(state, module, "ice_clip_use_native_activity", drb_ffi_ice_clip_use_native_activity_Binding, MRB_ARGS_REQ(1));
#endif
#if defined(ICE_CLIP_MICROSOFT)
    mrb_define_module_function_f(state, module, "ice_clip_use_window", drb_ffi_ice_clip_use_window_Binding, MRB_ARGS_REQ(1));
#endif
    mrb_define_module_function_f(state, module, "ice_clip_get", drb_ffi_ice_clip_get_Binding, MRB_ARGS_REQ(0));
    mrb_define_module_function_f(state, module, "ice_clip_set", drb_ffi_ice_clip_set_Binding, MRB_ARGS_REQ(1));
    mrb_define_module_function_f(state, module, "ice_clip_clear", drb_ffi_ice_clip_clear_Binding, MRB_ARGS_REQ(0));
